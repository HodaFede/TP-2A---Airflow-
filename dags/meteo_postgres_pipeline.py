...
DEFAULT_CONFIG = {
    ...
    "postgres_conn_id": "open_meteo_postgres",
    "weather_table": "weather_forecast",
    "audit_table": "weather_ingestion_audit",
    "start_date": None,
    "end_date": None,
}
...
def parse_dag_conf(dag_run: Any) -> Dict[str, Any]:
...
def extract_data(dag_run: Any = None) -> str:
    config = parse_dag_conf(dag_run)
...
def transform_data() -> str:
    ...
    for city_name, response in raw_records.items():
        if "daily" not in response:
            print(f"No daily.weather payload for {city_name}")
            continue
    ...
    print(f"Wrote prepared records to {PREPARED_FILE}")
    return str(PREPARED_FILE)


def ensure_postgres_tables(dag_run: Any = None) -> str:
    config = parse_dag_conf(dag_run)
    try:
    ...
    print(f"Ensured PostgreSQL tables: {config['weather_table']} and {config['audit_table']}")
    return config["postgres_conn_id"]


def load_data_to_postgres(dag_run: Any = None) -> Dict[str, Any]:
    config = parse_dag_conf(dag_run)
    ...
    if not rows:
        raise ValueError("Aucune donnée préparée disponible pour le chargement PostgreSQL.")
    ...
    print(f"Loaded {inserted} rows into PostgreSQL table {config['weather_table']}")
    return {
        "row_count": inserted,
        "city_count": city_count,
        "status": "success",
        "message": "Chargement PostgreSQL réalisé avec succès.",
    }


def track_ingestion(dag_run: Any = None, ti: Any = None) -> str:
    config = parse_dag_conf(dag_run)
    load_result = ti.xcom_pull(task_ids="load_data_to_postgres") or {}
    ...
    print(f"Wrote ingestion audit row: {status}, {row_count} rows, {city_count} cities")
    return "ok"


with DAG(
    dag_id="meteo_pipeline_postgres",
    start_date=datetime(2025, 1, 1),
    schedule="@daily",
    catchup=False,
    params=DEFAULT_CONFIG,
    tags=["tp", "airflow", "open-meteo", "postgres"],
) as dag:
    ...
    task_ensure_tables = PythonOperator(
        ...
        python_callable=ensure_postgres_tables,
        op_kwargs={"dag_run": "{{ dag_run }}"},
    )
